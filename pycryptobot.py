...
def executeJob(sc, app=PyCryptoBot(), trading_data=pd.DataFrame()):
    ...
    if len(df_last) > 0:
        ...
        if last_buy_amount > 0 and last_buy_price > 0 and price > 0 and last_action == 'BUY':
            ...
            if app.disableFailsafeFibonacciLow() is False and app.allowSellAtLoss() and app.sellLowerPcnt() is None and fib_low > 0 and fib_low >= float(price):
                action = 'SELL'
                last_action = 'BUY'
            ...
            if app.allowSellAtLoss() and app.trailingStopLoss() is not None and change_pcnt_high < app.trailingStopLoss():
                action = 'SELL'
                last_action = 'BUY'
            ...
            elif app.disableFailsafeLowerPcnt() is False and app.allowSellAtLoss() and app.sellLowerPcnt() is not None and margin < app.sellLowerPcnt():
                action = 'SELL'
                last_action = 'BUY'
            ...
            if app.disableProfitbankUpperPcnt() is False and app.sellUpperPcnt() is not None and margin > app.sellUpperPcnt():
                action = 'SELL'
                last_action = 'BUY'
...
def main(init=True):
    try:
        ...
        if init or app.isSimulation() == 1:
            ...
            trading_data = app.startApp(account, last_action, banner=init)
