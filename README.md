...
# Python Crypto Bot v6.4.1 (pycryptobot)
...
* "trailingbuypcnt" specifies the percentage for the price to increase before placing buy order after receiving buy signal. Eg: 0.5   (must be positive integer)
Note: the trailingbuy function will buy if the price is within 10% of the trailingbuypcnt setting.
* "trailingbuyimmediatepcnt" is a separate setting for percentage of price increase that can be used with "trailingimmediatebuy" or with the new Custom Strategies. (see separate explanation)
...
* "trailingsellpcnt" specifies the percentage for the price to decrease before placing sell order after receiving sell signal.  Eg: -0.5   (must be negative integer)
Note: the trailingsell function will sell if the price is within 10% of the trailingsellpcnt setting.
* "trailingsellimmediatepcnt" is a separate setting for percentage of price decrease that can be used with "trailingimmediatesell" or with the new Custom Strategies. (see separate explanation)   Eg: -1.5   (must be negative integer)
...
Trailing Sell is essentially the opposite of Trailing Buy.  It works when a sell signal is received and watches the price to make sure it's actually decreasing before selling.  Indicators can trigger a sell signal, but the price might only drop -0.25% and then start to rise again.  Rather than selling right away (candle close), the price point is recored at time the sell signal is received and monitored for decrease.  When the price drops to the "trailingsellpcnt" set point, the trailing sell is triggered and the bot will sell at candle close.  If the price increases above the set point, it is reset and starts tracking from there.  This will make sure you get as much profit as possible before selling.  If you watch charts and signals, you should be able to see the benefit to this setting over time.

Additionally "trailingimmediatesell" and "trailingsellimmediatepcnt" are more options that can be used with trailing sell.  Set "trailingsellimmediatepcnt" below "trailingsellpcnt" as an extra level of loss protection.  When the bot issues a sell signal, sometimes the price drops slowly, but many time the prices drops quickly and this will make sure the sell happens before candle close if the price is dropping rapidly.  "trailingimmediatesell" enables this feature and uses "trailingsellimmediatepcnt" as the set point.

Trailing Sell has one additional option, "trailingsellbailoutpcnt". This was added mostly for use with Custom Strategies because "trailingsellimmediatepcnt" is used differently with Custom Strategies, but can be used by itself, instead of requiring both "trailingimmediatesell" settings.  It works the same as "trailingimmediatesell", but only requires a negative percentage to be set with "trailingsellbailoutpcnt".  Think of this as a failsafe.  If by chance you had a buy and your bot is using 1 hour granularity and you get a sell signal just a few minutes after the top of the hour.  The bot will have to wait for candle close at the top of the next hour before it sells.  What if the price drops 25% within that hour?  Ouch!  That's a big loss.  Everyone has a different opinion of the set points, but it's not recommended to set this too close to 0.  A setting of -1.5 to -3 would probably be good.
...
With Dynamic Trailing Stoploss, both the trigger and the stoploss settings dynamically increase as the margin continues to increase.  Many times the more the margin increases (symbol price increases), you may see bigger drops or dips in the chart as the price continues to climb.  By dynamically adjusting, as your margin gets bigger, most people are willing to accept a littel bit bigger loss from the top before selling.  Because everyone has different opinions on what is a reasonable loss, that's why the multipliers are two different settings.
