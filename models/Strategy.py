...
class Strategy:
    ...
    def isBuySignal(
        ...
        if ( self.state.last_action == "WAIT"):
            log_text = (f"{str(now)} | {self.app.market} | {self.app.print_granularity()} | last_action is WAIT, do not buy yet")
            Logger.warning(log_text)
            return False
    ...
    def is_sell_trigger(
        ...
        if self.app.preventloss:
            if self.state.prevent_loss is False and margin > self.app.preventlosstrigger:
                ...
                Logger.warning(f"{self.app.market} - reached prevent loss trigger of {self.app.preventlosstrigger}%.  Watch margin ({self.app.preventlossmargin}%) to prevent loss.")
            elif (
                ...
                Logger.warning(f"{self.app.market} - time to sell before losing funds! Prevent Loss Activated!")
        ...
        if not self.app.sell_at_loss and margin <= 0:
            ...
            (self.app.no_sell_min_pcnt is not None) and (margin >= self.app.no_sell_min_pcnt)
        ) and (
            (self.app.no_sell_max_pcnt is not None) and (margin <= self.app.no_sell_max_pcnt)
        ):
            return False
        ...
        if debug:
            ...
            Logger.debug(f"margin >= self.app.no_sell_min_pcnt (margin: {margin})")
            Logger.debug(
                f"margin <= self.app.no_sell_max_pcnt (no_sell_max_pcnt: {self.app.no_sell_max_pcnt})"
            )
            Logger.debug("\n")
    ...
    def is_wait_trigger(self, margin: float = 0.0, goldencross: bool = False):
        ...
        if debug and self.state.action == "SELL":
            ...
            Logger.debug(f"self.state.action == 'SELL' (actual: {self.state.action})")
            Logger.debug(
                f"(self.app.no_sell_min_pcnt is not None (actual: {self.app.no_sell_min_pcnt})) and (margin ({margin}) >= self.app.no_sell_min_pcnt ({self.app.no_sell_min_pcnt}))"
            )
            Logger.debug(
                f"(self.app.no_sell_max_pcnt is not None (actual: {self.app.no_sell_max_pcnt})) and (margin ({margin}) <= self.app.no_sell_max_pcnt ({self.app.no_sell_max_pcnt}))"
            )
            Logger.debug("\n")
        ...
        if (
            self.state.action == "SELL"
            and (
                (self.app.no_sell_min_pcnt is not None)
                and (margin >= self.app.no_sell_min_pcnt)
            )
            and (
                (self.app.no_sell_max_pcnt is not None)
                and (margin <= self.app.no_sell_max_pcnt)
            )
        ):
            if not self.app.is_sim or (
                self.app.is_sim and not self.app.simresultonly
    ...
    def check_trailing_sell(self, state, price):
        ...
        # When all indicators signal strong sell and price decreases more than "self.app.trailingsellimmediatepcnt", immediate sell
            ...
            self.app.notify_telegram(waitpcnttext)
        # added 10% fluctuation to prevent holding another full candle for 0.025%
        elif pricechange > (self.app.trailingsellpcnt * .9):
            self.state.action = "WAIT"
        ...
        if debug:
            ...
            Logger.debug(f"Trailing Sell Triggered: {self.state.trailing_sell}  Wait Price: {self.state.waiting_sell_price} Current Price: {price} Price Chg: {_truncate(pricechange,2)} Immed Sell Pcnt: -{str(self.app.trailingsellimmediatepcnt)}%")
