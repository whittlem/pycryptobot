"""Remotely control your txbit account via their API"""
...
class AuthAPI(AuthAPIBase):
    def __init__(
        self, api_key="", api_secret="", api_passphrase="", api_url="https://api.txbit.io", cache_path="cache", use_cache=False, app: object = None
    ) -> None:
        """txbit API object model

        Parameters
        ----------
        api_key : str
            Your txbit account portfolio API key
        api_secret : str
            Your txbit account portfolio API secret
        api_url
            txbit API URL
        """
        ...
        if api_url not in valid_urls:
            raise ValueError("txbit API URL is invalid")
        ...
        if not p.match(api_key):
            self.handle_init_error("txbit API key is invalid", app=app)

        # validates the api secret is syntactically correct
        p = re.compile(r"^[A-z0-9-]{36,36}$")
        if not p.match(api_secret):
            self.handle_init_error("txbit API secret is invalid", app=app)
        ...
        if use_cache:
            ...
            self._cache_filepath = cache_path + os.path.sep + "txbit_order_cache.json"
            self._cache_lock_filepath = cache_path + os.path.sep + "txbit_order_cache.lock"

        self.usetxbitcache = use_cache
    ...
    def get_account(self, account: str) -> pd.DataFrame:
        ...
        if not p.match(self.account):
            self.handle_init_error("txbit account is invalid", app=self.app)
    ...
    def getMarkets(self) -> list:
        ...
        # GET /api/v1/symbols
        resp = self.auth_api("GET", "api/public/getmarkets")
    ...
    def auth_api(
        ...
        if method not in ["GET"]:
            raise TypeError("Method not GET.")
