...
def test_margin_binance():
    # using VET-GBP
    buy_size = 595.23
    buy_fee = 0.59523  # round(buy_size * 0.001, 8)
    buy_price = 0.1249
    buy_filled = 74.344227 # round(buy_size * buy_price, 8)
        
    debug = True
    sell_percent = 100
    sell_price = 0.1335
    sell_size = 79.463205 # round((sell_percent / 100) * (sell_price * buy_size), 8)
    sell_taker_fee = 0.001
    sell_filled = 79.38374179 # round((sell_size - round(sell_size * sell_taker_fee, 8)), 8)
    sell_fee = 0.0
    exchange = 'binance'
    # round(((buy_size - buy_fee) / buy_price), 8)

    expected_sell_fee = 0.07946321 # round(sell_size * sell_taker_fee, 8)
    expected_profit = 5.03951479 # round(sell_filled - buy_filled, 8)
    expected_margin = 6.77862289 # round((expected_profit / buy_filled) * 100, 8)
...
def test_calculate_negative_margin_on_binance_when_coin_price_over_1():
    ...
    expected_profit = -5.19319278 # round(sell_filled - buy_filled, 8)
    expected_margin = -11.61621257 # round((expected_profit / buy_filled) * 100, 8)
...
def test_calculate_negative_margin_on_binance_when_coin_price_over_1_2():
    ...
    expected_profit = -16.15646577 # round(sell_filled - buy_filled, 8)
    expected_margin = -16.80616243 # round((expected_profit / buy_filled) * 100, 8)
...
def test_calculate_negative_margin_on_binance_when_coin_price_under_1():
    ...
    expected_profit = -10.30756984 # round(sell_filled - buy_filled, 8)
    expected_margin = -11.70734815 # round((expected_profit / buy_filled) * 100, 8)
